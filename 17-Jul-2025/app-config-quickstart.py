## 16-Jul-2025
## Added new API endpoints (/, login, logout, features)
...
# Serve main feature page (after login)
@app.get("/features")
async def feature_page(request: Request):
    return templates.TemplateResponse("indexFeature.html", {"request": request})
...
async def index(request: Request):
    try:
        ...
        return templates.TemplateResponse("indexLogin.html", {"request": request, "flags": flags})
...
@app.get("/featureFlagEnable")
async def featureFlagEnable(key: str, flag: bool,config = Depends(get_app_config)):
    try:
        ...
        feature = client.get_configuration_setting(
            key=f".appconfig.featureflag/{key}", 
            label=None  # important!
        )
        # Deserialize, modify, and save it back
        feature_flag = FeatureFlagConfigurationSetting.deserialize(feature)
        feature_flag.enabled = flag
        
        # Serialize and save it back
        client.set_configuration_setting(feature_flag)
        
        return {"message": f"Feature flag '{key}' set to {flag}"}
    except Exception as e:
        # Catch and return the actual error for debugging
        return {"error": str(e)}

def mm(key: str, flag: bool, config = Depends(get_app_config)):
    try:
        # You can use the same connection_string you've already set globally
        client = AzureAppConfigurationClient.from_connection_string(connection_string)
        # Specify the label explicitly
...
async def get_all_feature_flags(config = Depends(get_app_config)):
    try:
        client = AzureAppConfigurationClient.from_connection_string(connection_string)
        ...
        for s in settings:
            if s.content_type and s.content_type.startswith("application/vnd.microsoft.appconfig.ff+json"):
                try:
                    ff = FeatureFlagConfigurationSetting.deserialize(s)
                    flags.append({
                        "key": ff.key,
                        "enabled": ff.enabled #,
                        #"label": ff.label or ""
                    })
                    print(f"Key: {ff.key}, Enabled: {ff.enabled}") #, Label: {ff.label or 'None'}")
                except Exception as e:
                    print(f"Error parsing flag {s.key}: {e}")
        return {"flags": flags}
...
async def update_feature_flag(
    ...
    try:
        ...
        # Format key
        if not req.key.startswith(".appconfig.featureflag/"):
            full_key = f".appconfig.featureflag/{req.key}"
        else:
            full_key = req.key

        # Get and update
        feature_flag = client.get_configuration_setting(key=full_key)
        if not feature_flag:
            return {"error": f"Feature flag {req.key} not found"}, 404

        ff = FeatureFlagConfigurationSetting.deserialize(feature_flag)
        ff.enabled = req.flag
        ff.content_type = "application/vnd.microsoft.appconfig.ff+json"

        # Create a new FeatureFlagConfigurationSetting or update existing one
        ff1 = FeatureFlagConfigurationSetting(
            key=req.key,
            enabled=req.flag,
            filters=[],  # empty filters = globally enabled/disabled
            description="Updated from API",
            label=None  # or specific label if used
        )

        # Save it with correct content type
        client.set_configuration_setting(ff1)
        verified = FeatureFlagConfigurationSetting.deserialize(
            client.get_configuration_setting(key=full_key)
        )

        return {
            "success": True,
            "message": f"Feature flag updated to {req.flag}",
            "verified_status": verified.enabled,
            "key": full_key
        }
